# Guiones completos — 6 Posts (Sentido Value)

Listos para grabar/publicar. Cada uno trae: **guion hablado** (reel), **texto en pantalla**, **caption**
y **palabra clave** para captura en n8n. Voz: cercana, chilena, anti-hype ("criterio, no humo").
Disclaimer sugerido al final de cada caption: *"Contenido educativo, no asesoría financiera. Toda inversión tiene riesgo."*

> Lead magnets nombrados (define el recurso real detrás de cada keyword antes de publicar):
> VALORA/CLASE → **Clase de Valoración paso a paso** · CHECKLIST → **Checklist de las 3 Preguntas** ·
> CRITERIO → **Clase/Sesión en vivo** · EMPRESAS → **Guía "Pocas pero bien"** · PATRIMONIO/TARDE → **Clase desde cero**.

---

## POST 1 · Valoración de empresa famosa · Reel · keyword `VALORA`

**Guion hablado (25–35 s) — tono amigable + intriga:**
> (0–3s, gancho) "Oye, déjame explicarte algo que casi nadie te dice: puedes conocer [Apple/Nvidia] perfecto… y aun así no tener idea si su acción está cara o barata."
> (3–10s) "Y voy a ser claro contigo: conocer la marca no es lo mismo que saber si vale lo que cuesta. Presta atención, porque acá está la clave."
> (10–24s) "Yo lo miro con 3 preguntitas, sin adivinar nada: primero, ¿cuánto gana de verdad? Su caja, no el número bonito del papel. Segundo, ¿cuánto está creciendo? Y tercero —y esta es la que casi nadie mira— ¿a qué precio se está vendiendo frente a todo eso?"
> (24–30s) "Y aquí viene lo bueno: con eso dejas de comprar por moda y empiezas a decidir tú. Lo mejor es que se aprende desde cero, aunque hoy no sepas nada de finanzas."
> (30–35s, CTA) "Cuéntame, ¿tú la ves cara o barata? Escríbeme VALORA y te paso la Clase de Valoración paso a paso. Ahí nos vemos 🙌"

**Texto en pantalla:** "¿[EMPRESA] está cara o barata?" → "1) ¿Cuánto gana? 2) ¿Cuánto crece? 3) ¿A qué precio?" → "Comenta VALORA 👇"

**Caption:**
> ¿[Empresa] está cara o barata? 📊
> Conoces la marca… pero eso no te dice si la acción vale lo que cuesta.
>
> Lo reviso con 3 preguntas, sin feeling:
> 1️⃣ ¿Cuánto gana de verdad? (flujo de caja libre)
> 2️⃣ ¿Cuánto crece?
> 3️⃣ ¿A qué precio se transa frente a eso?
>
> Con eso dejas de comprar por moda y decides tú.
> No es magia, es método — y se aprende desde cero.
>
> ¿La ves cara o barata? 👇
> Comenta **VALORA** y te envío la **Clase de Valoración paso a paso**.
> 📌 Guarda este post para tu próxima compra.
>
> Contenido educativo, no asesoría financiera. Toda inversión tiene riesgo.

---

## POST 2 · Mito de la diversificación / ETF · Reel · keyword `EMPRESAS`

**Guion hablado (25–35 s) — tono amigable + intriga:**
> (0–3s) "Déjame explicarte algo que te va a incomodar un poco: tener 30 acciones no te está protegiendo tanto como crees."
> (3–10s) "Y voy a ser honesto contigo: te vendieron que 'diversificar es menos riesgo'… y eso es solo media verdad. Espérate que te explico por qué."
> (10–20s) "Cuando tienes tantas empresas que ni te acuerdas qué hace la mitad, eso no es diversificar. Es tapar con cantidad que no las analizaste. ¿Me sigues?"
> (20–28s) "Y acá está la clave: los que la llevan de verdad hacen lo contrario. Eligen pocas, pero las entienden a fondo. No necesitas 30 apuestas… necesitas 5 o 6 decisiones que puedas defender."
> (28–33s, CTA) "Cuéntame acá abajo: ¿cuántas tienes… y cuántas entiendes de verdad? Y si quieres, escríbeme EMPRESAS y te enseño a elegir poquitas, pero bien 💪"

**Texto en pantalla:** "Tener 30 acciones NO te protege" → "El riesgo real: no entender lo que tienes" → "Comenta EMPRESAS 👇"

**Caption:**
> Te mintieron: tener 30 acciones no te protege. Te esconde que no sabes cuál es buena. 📉
>
> Diversificar tanto que ni sabes qué hace la mitad de tus empresas no es diversificar. Es tapar tu falta de análisis con cantidad.
>
> Los grandes inversores concentran en pocas empresas que entienden a fondo.
> No necesitas 30 apuestas. Necesitas 5 o 6 decisiones que sepas defender.
>
> ¿Cuántas acciones tienes… y cuántas entiendes de verdad? 👇
> Comenta **EMPRESAS** y te enseño a elegir pocas, pero bien.
> 📌 Guárdalo para tu próxima compra.
>
> Contenido educativo, no asesoría financiera.

---

## POST 3 · Historia personal + track record · Reel · keyword `CRITERIO`

**Guion hablado (30–40 s) — tono amigable + intriga:**
> (0–4s) "Voy a ser bien honesto contigo, y me da un poco de vergüenza: una vez vendí una gran empresa puro susto."
> (4–12s) "Déjame explicarte lo que pasó: el mercado cayó, me asusté y vendí… y después esa acción valía muchísimo más. No la perdí por mala suerte — la perdí porque no tenía criterio."
> (12–24s) "Y presta atención, porque esto lo cambió todo: me tomé en serio formarme. Invertí más de 15 mil dólares aprendiendo de gestores profesionales, y hoy llevo +148% acumulado versus el 103% del Nasdaq. No fue suerte, fue método."
> (24–34s) "Lo que aprendí es simple: cuando entiendes el negocio, una caída deja de asustarte… la aprovechas. Y esto lo puede aprender cualquiera, partiendo de cero."
> (34–40s, CTA) "Si te resuena y quieres invertir con criterio, escríbeme CRITERIO y te cuento cómo lo vemos juntos en Sentido Value 🙌"

**Texto en pantalla:** "Vendí en pánico porque no entendía lo que tenía" → "+148% acumulado vs 103% Nasdaq" → "Comenta CRITERIO 👇"

**Caption:**
> Vendí una gran empresa en pánico porque no entendía lo que tenía. 📉
>
> El mercado cayó, me asusté y vendí. Después valía mucho más.
> No la perdí por mala suerte: la perdí por no tener criterio.
>
> Eso me obligó a formarme en serio (+US$15.000 aprendiendo de gestores profesionales).
> Hoy llevo +148% acumulado frente al 103% del Nasdaq. No por suerte, por método.
>
> Cuando entiendes el negocio, una caída no te asusta: la aprovechas.
> Y eso se aprende desde cero.
>
> Comenta **CRITERIO** y te cuento cómo lo enseño en Sentido Value.
> *(Variante en vivo: "Comenta CRITERIO y te dejo el cupo para mi próxima clase en vivo.")*
>
> Contenido educativo, no asesoría financiera. Rentabilidades pasadas no garantizan resultados futuros.

> ⚠️ Datos verificados: +148% acumulado vs. 103% Nasdaq; +US$15.000 en formación. No inflar ni cambiar estas cifras.

---

## POST 4 · Carrusel checklist "3 preguntas" · Carrusel · keyword `CHECKLIST`

**Slides:**
- **Slide 1 (portada):** "Antes de comprar CUALQUIER acción, responde estas 3 preguntas." + "(Guarda este post 📌)"
- **Slide 2:** "1) ¿Genera caja de verdad? — Mira el flujo de caja libre, no solo la utilidad en papel. Una empresa puede 'ganar' y no generar efectivo."
- **Slide 3:** "2) ¿Tiene un foso? — Algo que la proteja de la competencia (marca, costos, red, patentes). Sin foso, los buenos números no duran."
- **Slide 4:** "3) ¿Está cara o barata? — Precio vs. valor: cuántos años de ganancias estás pagando por adelantado."
- **Slide 5 (cierre + CTA):** "Esto no es magia, es método. Y se aprende desde cero. 🔥 Comenta CHECKLIST y te envío el Checklist de las 3 Preguntas completo. 👉 Sígueme @sergiocortesinvesting"

**Caption:**
> Antes de comprar cualquier acción, responde estas 3 preguntas 👇 (guárdalo)
> 1️⃣ ¿Genera caja de verdad? (flujo de caja libre)
> 2️⃣ ¿Tiene un foso que la proteja?
> 3️⃣ ¿Está cara o barata frente a lo que vale?
>
> Con esto dejas de comprar por moda y empiezas a decidir con criterio.
> No es magia, es método — se aprende desde cero.
>
> 📌 Guarda este post.
> 🔥 Comenta **CHECKLIST** y te envío el **Checklist de las 3 Preguntas** completo.
>
> Contenido educativo, no asesoría financiera.

---

## POST 5 · Inflación / patrimonio (avatar profesional) · Reel · keyword `PATRIMONIO`

**Guion hablado (25–35 s) — tono amigable + intriga:**
> (0–4s) "Déjame explicarte algo que casi nadie te cuenta: puedes ganar súper bien todos los meses… y aun así quedarte sin patrimonio."
> (4–12s) "Y voy a ser claro contigo: tu plata en el banco se está encogiendo calladita. Cada año la inflación le quita valor — lo que hoy compras con 100, mañana te cuesta más."
> (12–22s) "Ahora presta atención, porque acá está el giro: si tu plata está quieta, no estás ahorrando… te estás empobreciendo sin darte cuenta. Pero bien invertida, esa misma plata empieza a crecer sola, año tras año."
> (22–30s) "Y ojo, no es que ganes poco. Es que nadie te enseñó a que tu plata trabaje por ti — sobre todo si eres profesional, te va bien, pero nunca te metiste en esto."
> (30–35s, CTA) "Cuéntame: ¿tu plata trabaja para ti o se encoge en el banco? Escríbeme PATRIMONIO y te paso una clase para empezar desde cero, tranqui, sin tecnicismos 🙌"

**Texto en pantalla:** "Ganas bien pero no construyes patrimonio" → "Tu plata quieta se encoge (inflación)" → "Comenta PATRIMONIO 👇"

**Caption:**
> Puedes ganar bien todos los meses y aun así quedarte sin patrimonio. 📉
>
> Tu plata en el banco se encoge en silencio: cada año la inflación le quita valor.
> Si está quieta, no estás ahorrando… te estás empobreciendo sin darte cuenta.
>
> Bien invertido, ese mismo dinero crece sobre sí mismo, año tras año.
> No es que ganes poco. Es que nadie te enseñó a que tu dinero trabaje por ti.
>
> ¿Tu dinero trabaja para ti o se encoge en el banco? 👇
> Comenta **PATRIMONIO** y te envío la clase para empezar desde cero.
> 📌 Guárdalo para la próxima vez que dejes plata parada.
>
> Contenido educativo, no asesoría financiera.

---

## POST 6 · Trampa de valor / error caro · Reel · keyword `TRAMPA`

**Guion hablado (25–35 s) — tono amigable + intriga:**
> (0–4s) "Déjame explicarte algo antes de que compres tu próxima acción 'barata': muchas veces no es una oferta… es una trampa."
> (4–12s) "Y voy a ser claro contigo: el precio bajo engaña. Mucha gente compra 'barato' pensando que hizo el negocio del año… y en realidad compró una empresa que se cae a pedazos."
> (12–24s) "Presta atención, porque acá está la diferencia: barato de verdad es cuando el precio está por debajo de lo que la empresa vale, no solo porque bajó. ¿Cómo lo notas? Mirando el negocio: ¿sigue generando caja?, ¿tiene futuro?, ¿por qué está tan barata?"
> (24–30s) "Al final es simple: el precio no te dice si vale. El negocio sí."
> (30–35s, CTA) "Cuéntame, ¿te ha pasado comprar 'barato' y salir perdiendo? Escríbeme TRAMPA y te muestro cómo diferenciar una oportunidad de una trampa 🙌"

**Texto en pantalla:** "Barato ≠ oferta" → "A veces es una trampa de valor" → "Comenta TRAMPA 👇"

**Caption:**
> Una acción barata no siempre es una oferta. A veces es una trampa que te cuesta caro. ⚠️
>
> El precio bajo engaña: muchos compran "barato" y en realidad compran un negocio que se deteriora.
> Barato de verdad es cuando el precio está por debajo de lo que la empresa vale — no cuando simplemente cayó.
>
> Para distinguirlo, mira el negocio: ¿sigue generando caja? ¿tiene futuro? ¿por qué está barata?
> El precio no te dice si vale. El negocio sí.
>
> ¿Te ha pasado comprar "barato" y salir perdiendo? 👇
> Comenta **TRAMPA** y te muestro cómo distinguir una oportunidad de una trampa.
>
> Contenido educativo, no asesoría financiera.

---

## Bonus (opcional) · Ángulo "no es tarde" · keyword `TARDE`

Detectado en la competencia (@invertirdesdecero, @santinversor) con alto engagement. Úsalo como rotación.
- **Hook:** "Cada año que esperas para invertir te cuesta más de lo que crees."
- **Idea:** el costo de oportunidad de postergar (interés compuesto) + "no es tarde, es hoy".
- **CTA:** "Comenta TARDE y te mando la clase para empezar ahora, aunque partas desde cero."

---

## Recordatorio de captura (n8n)

| Post | Keyword | Lead magnet a preparar |
|---|---|---|
| 1 | VALORA | Clase de Valoración paso a paso |
| 2 | EMPRESAS | Guía "Pocas pero bien" |
| 3 | CRITERIO | Clase/sesión en vivo (o info del programa) |
| 4 | CHECKLIST | Checklist de las 3 Preguntas |
| 5 | PATRIMONIO | Clase para empezar desde cero |
| 6 | TRAMPA | Mini-clase "oportunidad vs. trampa" |
| bonus | TARDE | Clase para empezar ahora |

Cada keyword debe apuntar a un recurso/DM distinto para que n8n sepa qué post generó cada lead.

---

## Banco de frases de intriga (open loops) — reutilizables

Úsalas al inicio o en las transiciones para generar el "¿qué me va a decir?" y retener la atención:
- "Déjame explicarte algo que casi nadie te dice…"
- "Voy a ser claro/honesto contigo…"
- "Presta atención, porque acá está la clave…"
- "Y aquí viene lo que cambia todo…"
- "Espérate, porque esto te va a hacer clic…"
- "Te lo digo derecho…"
- "Y ojo con esto, porque casi nadie lo mira…"
- "Ahora viene el giro…"
- "Lo que voy a decirte te va a incomodar un poco, pero es real…"

**Regla de uso:** máximo 1–2 por reel (al gancho + una transición). Abusar de ellas suena a vendedor.
Mantener siempre el fondo anti-hype y el criterio; la intriga engancha, el contenido cumple la promesa.
